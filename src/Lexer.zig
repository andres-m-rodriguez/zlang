...
pub fn scan(self: *Self) ?Token {
    ...
    if (c == ':') {
        self.cursor += 1;
        return Token.init(":", Token.TokenKind.Colon);
    }
    if (c == ',') {
        self.cursor += 1;
        return Token.init(",", Token.TokenKind.Comma);
    }
    ...
    if (isIdentCont(self.source[self.cursor])) {
        ...
        if (keywordKind(word)) |kind| {
            return Token.init(word, kind);
        }
        return Token.init(word, .Identifier);
    }

    self.cursor += 1;
    const slice: []const u8 = &[_]u8{c};
    return Token.init(slice, .Unknown);
}
...
fn scanString(self: *Self) Token {
    ...
    if (self.cursor >= self.source.len) {
        return Token.init(self.source[open .. open + 1], .Unknown);
    }
    const inner = self.source[start..self.cursor];
    self.cursor += 1;
    return Token.init(inner, .String);
}
...
fn parseDigit(value: []const u8) ?Token {
    var idx: usize = 0;
    while (idx < value.len) : (idx += 1) {
        if (!ascii.isDigit(value[idx])) break;
    }
    if (idx == 0) return null;
    return Token.init(value[0..idx], .Number);
}
...
fn parseDelimiter(value: []const u8) ?Token {
    ...
    return Token.init(value[0..1], kind);
}

fn parseOperator(value: []const u8) ?Token {
    if (value.len == 0) return null;
    if (!isOperator(value[0])) return null;

    if (value.len >= 2) {
        const two = value[0..2];
        if (std.mem.eql(u8, two, "==")) return Token.init(two, .EqualEqual);
        if (std.mem.eql(u8, two, "!=")) return Token.init(two, .BangEqual);
        if (std.mem.eql(u8, two, "<=")) return Token.init(two, .LessEqual);
        if (std.mem.eql(u8, two, ">=")) return Token.init(two, .GreaterEqual);
    }
